...
Файл может иметь сколь угодно большой объём, но должен быть разделён строками согласно тому, как это делает CAN dump. Строки хранятся в памяти целиком до момента завершения их анализа.

Нет как такового обнаружения формата строки. Любая таблица с допустимой третьей колонкой будет принята фильтром. Обнаружение строки, доколе можно сделать, многократно замедлит анализ.